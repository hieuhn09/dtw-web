...
UX round 2 (2026-07-31, owner): homepage teaser — "filter by use case →" hint removed (duplicated the "Open full dashboards →" button); teaser columns expanded to `# / Model / General / Reason / Code / Math / Vision / Price` (+Math/+Vision, reusing the main table's exact header strings); "Input $/M" replaced with a blended "Price" column (`(4×input + 1×output)/5`, falling back to whichever side is non-null, "–" when both are null); teaser now shows the top 5 rows (was top 4). Dashboard table — bar width re-scaled to `floor = columnMin − 10`, `width% = (v − floor)/(columnMax − floor) × 100` (100% when the column has no spread); bars now carry a value-band fill color (`delta = columnMax − v`: ≤2 → `var(--up)`, ≤6 → `var(--bar)`, else → `var(--amber)`) with the number always shown so color is never the only signal; the score value now centers ABOVE a full-width bar (never inside the fill) and the whole stack centers in its column so values line up vertically; every sortable header except `#`/Model gained a small ⓘ tooltip (title + aria-label, translated EN/VI/ID, rendered outside the sort `<button>` so it never triggers a sort). Methodology copy rewritten in plain language (TrueSkill-as-Xbox-matchmaking framing, conservative-estimate explanation, category list, LLM Stats + weekly-Monday refresh) — updated in the seed script, the `getDashboardMethodology()` fallback, and the live DB global (via a one-off scratch script, deleted after use). The "For informational purposes only · not investment or procurement advice" disclaimer line is no longer rendered under the methodology card (the CMS `disclaimer` field/schema is untouched, just unused in the UI now).

Round 3 (owner): working CSS tooltips replace native title (instant, touch-friendly); teaser rank 1-3 accent-colored, per-column top-3 score chips (amber tiers), numeric columns center-aligned, ⓘ tooltips incl. price-ratio explanation.
