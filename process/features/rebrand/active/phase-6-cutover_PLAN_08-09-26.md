...
**Status**: 🧪 TESTING — cutover đã đi qua 10-09-26 — xem reports/phase-6-cutover_REPORT_10-09-26.md; CÒN TREO sameAs, GA4, Resend verify (cập nhật 10-09-26). Central tenant row (`domain`/`frontendUrl`) đã sửa xong 10-09-26, nhưng lần sửa tay đó VÔ Ý đổi luôn `slug` `dtw`→`otw` (trái D11 "never change after launch", umbrella §5.3) lúc 10:04Z — hậu quả: content-engine nhận `400 unknown publicationId` từ Central suốt 10→14-09-26, 31 bài dtw `publish_failed`. Đã khôi phục `slug='dtw'` + đăng bù đúng ngày 14-09-26 — xem `content-engine` `process/features/dtw-integration/reports/central-tenant-slug-outage_REPORT_14-09-26.md` (cập nhật 14-09-26)
