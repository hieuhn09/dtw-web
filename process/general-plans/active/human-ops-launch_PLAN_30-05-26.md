...
- Next: **P0-B** (admin bootstrap + CMS-driven pillar routing).
- 2026-06-01: P0-D scope chosen = make-real the cheap surfaces + de-fake the rest. Chunk 1 done & verified: (a) **Corrections** `/trust/corrections` now reads the Corrections collection (server component + `getCorrections()`), honest empty state, verified a real correction renders; (b) **Wire Drops** fake `setInterval`/SAMPLES stream removed (renders only real CMS drops; false "Realtime · WebSocket" kicker + non-functional RSS/more affordances dropped). Branch `feat/p0d-integrity`. Remaining P0-D: Search (real DB-backed), Studio/Briefing honest copy, Dashboards de-fake + "coming soon".
- 2026-06-01: P0-D **complete**. Chunk 2: studio/briefing dropped "wired through CMS" → honest copy; dashboards (page + funding-tracker + ai-leaderboard + homepage teaser) dropped all false "Live/15 min/138 instruments/18 models tracked" → "Preview · sample data · coming soon". Chunk 3: **Search made real** — `searchArticles()` (Postgres `like` on title/dek, published-only, no Meilisearch) + `runSearch` server action; `/search` page + ⌘K overlay rewritten to debounce-call the action (mock ARTICLES removed); dropped fake "134ms"/"Powered by Meilisearch"/"we log empty queries"; removed non-functional date/type facets (pillar facet wired client-side). Verified query: Baidu→1, Singapore→2, Oppo→1, nomatch→0. typecheck + build green. **All P0-D surfaces now honest or real.**
