// ========= EDITORIAL COVER ART =========
// Abstract geometric compositions used in place of photos.
// Deterministic from a seed (article id) so each story gets the same cover every render.
...
const ART_PALETTES = {
  ...
  asia: [["#7C2D12", "#E04E1F", "#FDE68A", "#FFFFFF"], ["#431407", "#F97316", "#FCD34D", "#1F2937"]],
  dev: [["#052E16", "#16A34A", "#86EFAC", "#FFFFFF"], ["#0F172A", "#10B981", "#A7F3D0", "#F3F4F6"]],
  products: [["#451A03", "#D97706", "#FCD34D", "#FFFFFF"], ["#1F2937", "#F59E0B", "#FDE68A", "#FAFAF7"]],
  policy: [["#0F172A", "#475569", "#94A3B8", "#F1F5F9"], ["#111827", "#64748B", "#CBD5E1", "#FFFFFF"]]
};

function pickPalette(pillar, seed) {
  const list = ART_PALETTES[pillar] || ART_PALETTES.policy;
  return list[seed % list.length];
}

function CoverArt({ pillar = "asia", seed = "x", label, height = 240, variant, style = {} }) {
  ...
  if (v === 0) {
    ...
    body =
    <>
        <rect x="0" y="0" width={w} height={hh} fill={c1} style={{ height: "480px" }} />
        <g transform={`translate(${80 + h % 60} ${60}) rotate(-8)`}>
          <rect x="0" y="0" width={520} height={64} fill={c3} opacity=".9" />
          <rect x="0" y="84" width={420} height={64} fill={c2} />
          <rect x="0" y="168" width={580} height={64} fill={c4} opacity=".85" />
          <rect x="0" y="252" width={360} height={64} fill={c2} opacity=".6" />
        </g>
        <circle cx={w - 100} cy={hh - 90} r="70" fill={c2} opacity=".6" />
      </>;
    ...
    <>
        ...
        <circle key={i} r={r} fill="none" stroke={i % 2 ? c2 : c1} strokeWidth={i === 0 ? 2 : 8} opacity={0.85 - i * 0.1} />
        )}
          <circle r="20" fill={c2} />
        </g>
    ...
    <>
        ...
        <rect x="0" y={hh * 0.55} width={w * 0.35} height={hh * 0.45} fill={c2} opacity=".7" />
        <rect x={w * 0.35} y={hh * 0.55} width={w * 0.65} height={hh * 0.45} fill={c3} opacity=".55" />
        <circle cx={w * 0.35} cy={hh * 0.55} r="58" fill={c4} />
      </>;

  } else if (v === 3) {
    // Halftone gradient grid
    body =
    <>
      ...
      Array.from({ length: 14 }).map((__, col) => {
        ...
        const rad = Math.max(2, 22 - dist * 0.03);
        return <circle key={r + "_" + col} cx={cx} cy={cy} r={rad} fill={c3} opacity={0.85 - dist * 0.001} />;
      })
      )}
    ...
    <>
        ...
        <line key={i} x1="30" x2={w - 30} y1={80 + i * 70} y2={80 + i * 70} stroke={c1} strokeOpacity=".1" />
        )}
        </g>
        <path d={toPath(seriesA)} fill="none" stroke={c2} strokeWidth="4" strokeLinecap="round" />
        <path d={toPath(seriesB)} fill="none" stroke={c1} strokeWidth="2.5" strokeDasharray="6 6" />
        <rect x={w - 220} y="40" width="180" height="60" fill={c1} />
        <text x={w - 200} y="80" fill={c4} fontFamily="IBM Plex Mono, monospace" fontSize="22" fontWeight="600">+14.2%</text>
        <text x={w - 200} y="115" fill={c1} fontFamily="IBM Plex Mono, monospace" fontSize="11">vs prev. 30d</text>
      </>;
    ...
    <>
        ...
        <rect x={w * 0.18} y={hh * 0.7} width={w * 0.82} height={hh * 0.3} fill={c3} opacity=".8" />
        <text x={w * 0.22} y={hh * 0.62} fill={c4}
      fontFamily="Source Serif 4, serif" fontWeight="700"
      fontSize="280" letterSpacing="-18">
          {glyph}
        </text>
        <line x1={w * 0.18} y1={hh * 0.7} x2={w} y2={hh * 0.7} stroke={c4} strokeWidth="2" />
      </>;

  }

  return (
    <div style={{ position: "relative", height, overflow: "hidden", borderRadius: 0, ...style }}>
      <svg viewBox={`0 0 ${w} ${hh}`} preserveAspectRatio="xMidYMid slice" width="100%" height="100%" style={{ display: "block" }}>
        {body}
        {/* grain */}
...
function BrandMark({ name, size = 18, monospace = true, bg, fg }) {
  return (
    <span style={{
      ...
      fontSize: size * 0.65, fontWeight: 600, letterSpacing: ".02em"
    }}>
      <span style={{ width: 8, height: 8, background: fg || "var(--paper)", borderRadius: 1, opacity: .8 }} />
      {name}
    </span>);
...
function CityChip({ city, size = 11 }) {
  ...
  return (
    <span className="mono" style={{
      ...
      fontSize: size, fontWeight: 600, color: "var(--ink)",
      textTransform: "uppercase", letterSpacing: ".08em"
    }}>
      <span style={{ width: 6, height: 6, background: c, borderRadius: 99, display: "inline-block" }} />
      {city}
    </span>);

}

Object.assign(window, { CoverArt, Avatar, BrandMark, CityChip, ART_PALETTES });