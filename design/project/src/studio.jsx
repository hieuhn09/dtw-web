// ============ DTW STUDIO PAGE ============
// Branded-content arm. Explains what Studio is, how it works under disclosure,
// and routes every CTA to partnership@dailytechwire.com.
function StudioPage({ navigate }){
  ...
  const principles = [
    [ "check", t("Always labelled", "Luôn gắn nhãn", "Selalu diberi label"),
      t("Every Studio piece carries a yellow background, a 'Paid Partner' tag, and a disclosure at the top, middle, and end. Readers never have to guess.",
    ...
    [ "lock", t("Separate from the newsroom", "Tách biệt với toà soạn", "Terpisah dari ruang redaksi"),
      t("Studio is a distinct team with its own budget. Our reporters and editors are never assigned to sponsored work, and partners never see unpublished newsroom coverage.",
    ...
    [ "info", t("Clear on the line", "Rạch ròi giới hạn", "Jelas batasannya"),
      t("Partners choose the topic and check their own product facts. They can't approve copy that names competitors, pick pull-quotes, or require a sales pitch.",
    ...
    <div className="container" style={{paddingTop:32, paddingBottom:72, maxWidth:980}}>
      ...
      <section style={{
        ...
        <div style={{position:"relative", maxWidth:680}}>
          ...
          <h1 className="serif" style={{margin:"0 0 18px", fontSize:52, fontWeight:650, letterSpacing:"-0.03em", lineHeight:1.05, color:"#FFFFFF", textWrap:"balance"}}>
            {t("Storytelling for brands, held to a newsroom standard",
               "Kể chuyện cho thương hiệu, theo chuẩn của một toà soạn",
               "Bercerita untuk merek, dengan standar ruang redaksi")}
          </h1>
          <p className="serif" style={{margin:"0 0 30px", fontSize:19, lineHeight:1.55, color:"rgba(232, 237, 247, .82)", textWrap:"pretty"}}>
            {t("DTW Studio is our branded-content team. We work with partners to tell their story to an audience that reads us for tech across Asia and the world, researched, written, and produced with the same care as our journalism, and labelled clearly so trust stays intact on both sides.",
          ...
          <a href={`mailto:${EMAIL}?subject=DTW%20Studio%20inquiry`} style={{
            display:"inline-flex", alignItems:"center", gap:10,
            padding:"14px 24px", background:"var(--accent)", color:"#fff",
            borderRadius:8, fontSize:15, fontWeight:600, textDecoration:"none"
          }}>
            <Icon name="mail" size={16} color="#fff" /> {t("Start a project", "Bắt đầu dự án", "Mulai proyek")}
          </a>
        </div>
      </section>
      ...
      <div style={{display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:20, marginBottom:48}}>
        {services.map(s=>(
          <div key={s.label} style={{
            ...
            <div className="serif" style={{fontSize:19, fontWeight:600, lineHeight:1.25, letterSpacing:"-0.01em"}}>{s.label}</div>
            <p style={{margin:0, fontSize:13.5, lineHeight:1.6, color:"var(--ink-2)"}}>{s.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <h2 className="serif" style={{margin:"0 0 8px", fontSize:30, fontWeight:650, letterSpacing:"-0.02em", lineHeight:1.1, textWrap:"balance"}}>
          {t("Sponsored, and never hidden", "Tài trợ, và không bao giờ giấu giếm", "Bersponsor, dan tak pernah disembunyikan")}
        </h2>
        <p className="serif text-mute" style={{margin:"0 0 28px", fontSize:16, lineHeight:1.55, maxWidth:680}}>
          {t("The reason partners reach our readers is that those readers trust us. We protect that with a few firm rules, the same ones whether the partner is a startup or a global cloud provider.",
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:28}}>
          {principles.map(([icon, head, body])=>(
            <div key={head} style={{display:"flex", flexDirection:"column", gap:8}}>
              <div style={{display:"flex", alignItems:"center", gap:10}}>
                <div style={{
                  ...
                  <Icon name={icon} size={16} color="var(--accent-ink)" stroke={2.2} />
                </div>
                <div className="serif" style={{fontSize:16, fontWeight:600, lineHeight:1.2}}>{head}</div>
              </div>
              <p style={{margin:0, fontSize:13, lineHeight:1.6, color:"var(--ink-2)"}}>{body}</p>
            </div>
          ))}
        </div>
        <div style={{marginTop:24, paddingTop:18, borderTop:"1px solid var(--hair)"}}>
          <a onClick={()=>navigate("/trust/sponsored")} className="linkish" style={{fontSize:13, fontWeight:600, color:"var(--accent)", cursor:"pointer", display:"inline-flex", alignItems:"center", gap:6}}>
            {t("Read the full Sponsored & Affiliate policy", "Đọc đầy đủ Chính sách Tài trợ & Affiliate", "Baca lengkap Kebijakan Sponsor & Afiliasi")}
      ...
      <div style={{
        ...
        padding:"34px 40px", background:"var(--surface)",
        border:"1px solid var(--hair-2)", borderRadius:12
      }}>
        <div>
          <h2 className="serif" style={{margin:"0 0 8px", fontSize:26, fontWeight:650, letterSpacing:"-0.02em", lineHeight:1.15, textWrap:"balance"}}>
            {t("Have a story worth telling well?", "Có một câu chuyện đáng kể tử tế?", "Punya kisah yang layak diceritakan dengan baik?")}
          </h2>
          <p style={{margin:0, fontSize:14, lineHeight:1.6, color:"var(--ink-2)", maxWidth:460}}>
            {t("Tell us who you're trying to reach and what you want them to understand. We'll come back with a shape, a timeline, and a quote, usually within two business days.",
        ...
        <div style={{display:"flex", flexDirection:"column", gap:12, alignItems:"flex-start"}}>
          <a href={`mailto:${EMAIL}?subject=DTW%20Studio%20inquiry`} className="accent-glow" style={{
