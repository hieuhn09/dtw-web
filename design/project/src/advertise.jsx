// ============ ADVERTISE PAGE ============
// /advertise, media & advertising opportunities. Distinct from DTW Studio
// (branded content): this is the rate-card / formats / audience-reach page.
// Every CTA routes to advertising@dailytechwire.com.
function AdvertisePage({ navigate }) {
  ...
  // Headline audience numbers, kept few and meaningful.
  const stats = [
    { v: "2.4M", k: t("Monthly readers", "Độc giả mỗi tháng", "Pembaca bulanan"),
      s: t("across web & apps", "trên web & ứng dụng", "lewat web & aplikasi") },
    { v: "210K", k: t("Newsletter subscribers", "Người đăng ký bản tin", "Pelanggan newsletter"),
      s: t("six titles · double opt-in", "sáu bản tin · xác nhận kép", "enam newsletter · opt-in ganda") },
    { v: "4:50", k: t("Avg. read time", "Thời gian đọc TB", "Rata-rata waktu baca"),
      s: t("minutes per session", "phút mỗi phiên", "menit per sesi") },
    { v: "63%", k: t("Senior decision-makers", "Cấp ra quyết định", "Pengambil keputusan senior"),
      s: t("director level and above", "từ cấp giám đốc trở lên", "level direktur ke atas") },
  ];

  const markets = ["Singapore", "Jakarta", "Tokyo", "Seoul", "Bengaluru", "Hanoi", "San Francisco", "London"];

  // Reasons to advertise, three firm principles, mirrors Studio's "How we work".
  const why = [
    ["lock", t("Brand-safe by design", "An toàn thương hiệu", "Aman untuk merek"),
      t("No programmatic exchanges, no autoplay video, no tracking-pixel free-for-all. Every placement sits beside reporting we are proud of, and nothing else.",
    ...
    ["user", t("An audience that pays attention", "Độc giả thật sự chú tâm", "Audiens yang menyimak"),
      t("Our readers are founders, engineers, investors, and policymakers who arrive on purpose and stay to read. They are the people your category actually needs to reach.",
    ...
    ["globe", t("Asia and the world", "Châu Á và thế giới", "Asia dan dunia"),
      t("Run a campaign in one city or across every market we cover. Reporting from in-region desks means your message lands in the right language and the right context.",
  ...
  // Inventory / formats.
  const formats = [
    { icon: "mail", name: t("Newsletter sponsorship", "Tài trợ bản tin", "Sponsor newsletter"),
      desc: t("A single host-read placement in the AM Brief, AI Weekly, or any of our six titles. One sponsor per send.",
    ...
    { icon: "globe", name: t("Homepage & display", "Trang chủ & hiển thị", "Beranda & display"),
      desc: t("Premium positions on the homepage and across article pages. Flat-rate takeovers or standard CPM, viewability-guaranteed.",
    ...
    { icon: "headphone", name: t("Podcast & audio", "Podcast & âm thanh", "Podcast & audio"),
      desc: t("Host-read spots in our shows, plus audio versions of every article. Pre-roll, mid-roll, or full episode presenting sponsor.",
    ...
    { icon: "user", name: t("Events & roundtables", "Sự kiện & toạ đàm", "Acara & roundtable"),
      desc: t("Closed-door dinners, panels, and briefings in the cities we report from. Curated guest lists, on or off the record.",
    ...
    { icon: "data", name: t("Dashboards sponsorship", "Tài trợ bảng dữ liệu", "Sponsor dasbor"),
      desc: t("Category-exclusive presence on our live Funding Tracker and AI Leaderboard, the tools our readers check daily.",
    ...
    { icon: "feather", name: t("Branded content", "Nội dung thương hiệu", "Konten bermerek"),
      desc: t("Long-form features, research, and video told to a newsroom standard, produced and clearly labelled by DTW Studio.",
      ...
      note: t("via DTW Studio →", "qua DTW Studio →", "lewat DTW Studio →"), to: "/studio" },
  ];
  ...
  return (
    <div className="container" style={{ paddingTop: 32, paddingBottom: 72, maxWidth: 980 }}>
      ...
      <section style={{
        ...
        borderRadius: 14, padding: "60px 48px", marginBottom: 44
      }}>
        <GridBackdrop color="rgba(232, 237, 247, .06)" size={40} fadeRadius="85%" />
        <div style={{ position: "absolute", right: -100, bottom: -100, width: 340, height: 340, borderRadius: "50%", background: "var(--accent)", opacity: .15, filter: "blur(110px)", pointerEvents: "none" }} />
        <div style={{ position: "relative", maxWidth: 700 }}>
          ...
          <h1 className="serif" style={{ margin: "0 0 18px", fontSize: 54, fontWeight: 650, letterSpacing: "-0.03em", lineHeight: 1.04, color: "#FFFFFF", textWrap: "balance" }}>
            {t("Reach the people building tech across Asia and the world",
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <a href={mailto} className="accent-glow" style={{
              ...
              <Icon name="download" size={16} color="#fff" /> {t("Request the media kit", "Nhận media kit", "Minta media kit")}
            </a>
            <a href={mailto} style={{
              display: "inline-flex", alignItems: "center", gap: 10,
              padding: "14px 22px", background: "transparent", color: "#E8EDF7",
              border: "1px solid rgba(232, 237, 247, .24)",
              borderRadius: 8, fontSize: 15, fontWeight: 600, textDecoration: "none"
            }}>
              <Icon name="mail" size={16} color="#E8EDF7" /> {t("Talk to the team", "Trao đổi với đội ngũ", "Bicara dengan tim")}
            </a>
          </div>
        </div>
        ...
        {/* Stats strip */}
        <div style={{
          position: "relative", marginTop: 44, paddingTop: 30,
          borderTop: "1px solid rgba(232, 237, 247, .14)",
          display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 28
        }}>
          {stats.map((st) =>
            <div key={st.k}>
              <div className="serif" style={{ fontSize: 38, fontWeight: 650, letterSpacing: "-0.02em", color: "#FFFFFF", lineHeight: 1 }}>{st.v}</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#E8EDF7", marginTop: 8 }}>{st.k}</div>
              <div style={{ fontSize: 11.5, color: "rgba(232, 237, 247, .58)", marginTop: 2 }}>{st.s}</div>
            </div>
          )}
        </div>
      </section>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20, marginBottom: 48 }}>
        {why.map(([icon, head, body]) =>
          <div key={head} style={{
            display: "flex", flexDirection: "column", gap: 12, padding: 26,
            ...
            <div className="serif" style={{ fontSize: 19, fontWeight: 600, lineHeight: 1.25, letterSpacing: "-0.01em" }}>{head}</div>
            <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.6, color: "var(--ink-2)" }}>{body}</p>
          </div>
        )}
      </div>
      </Reveal>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 18, paddingBottom: 10, borderBottom: "2px solid var(--brand-navy)" }}>
        <div>
          <div className="kicker" style={{ marginBottom: 4 }}>{t("Formats & placements", "Định dạng & vị trí", "Format & penempatan")}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20, marginBottom: 48 }}>
        {formats.map((f) =>
          <div key={f.name}
            ...
            className={f.to ? "card-hover" : undefined}
            style={{
              display: "flex", flexDirection: "column", gap: 12, padding: 24,
              background: "var(--surface)", border: "1px solid var(--hair)", borderRadius: 10,
              cursor: f.to ? "pointer" : "default"
            }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <span className="mono" style={{
                ...
                border: "1px solid var(--hair-2)", background: "var(--paper)", whiteSpace: "nowrap"
              }}>{f.note}</span>
            </div>
            <div className="serif" style={{ fontSize: 18, fontWeight: 600, lineHeight: 1.25, letterSpacing: "-0.01em" }}>{f.name}</div>
            <p style={{ margin: 0, fontSize: 13, lineHeight: 1.6, color: "var(--ink-2)" }}>{f.desc}</p>
          </div>
        )}
      </div>
      </Reveal>
      ...
      <Reveal>
      <div style={{
        padding: "36px 40px", background: "var(--surface-2)",
        border: "1px solid var(--hair)", borderRadius: 12, marginBottom: 44,
        display: "grid", gridTemplateColumns: "1fr 1.3fr", gap: 48, alignItems: "center"
      }}>
        <div>
          <div className="kicker muted" style={{ marginBottom: 8 }}>{t("Who reads DTW", "Ai đọc DTW", "Siapa pembaca DTW")}</div>
          <h2 className="serif" style={{ margin: "0 0 12px", fontSize: 28, fontWeight: 650, letterSpacing: "-0.02em", lineHeight: 1.12, textWrap: "balance" }}>
            {t("A senior, technical audience, by role", "Độc giả cấp cao, kỹ thuật, theo vai trò", "Audiens senior dan teknis, menurut peran")}
          </h2>
          <p className="serif text-mute" style={{ margin: "0 0 16px", fontSize: 15, lineHeight: 1.55 }}>
            {t("Self-reported on sign-up across our subscriber base. Reading and event audiences skew more senior still.",
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {markets.map((m) =>
              <span key={m} className="mono" style={{
                ...
                border: "1px solid var(--hair-2)", background: "var(--paper)", color: "var(--ink-2)"
              }}>{m}</span>
            )}
          </div>
        </div>
          ...
          {audience.map((a) =>
            <div key={a.k}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
                ...
                <span className="mono tnum" style={{ fontSize: 13, fontWeight: 600, color: a.c }}>{a.v}%</span>
              </div>
              <div style={{ height: 8, background: "var(--hair)", borderRadius: 99, overflow: "hidden" }}>
                <div style={{ width: a.v * 3 + "%", maxWidth: "100%", height: "100%", background: a.c, borderRadius: 99 }} />
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{
        ...
        <div>
          <div className="serif" style={{ fontSize: 18, fontWeight: 600, marginBottom: 6, letterSpacing: "-0.01em" }}>
            {t("Advertising is always separate from the newsroom", "Quảng cáo luôn tách biệt với toà soạn", "Iklan selalu terpisah dari ruang redaksi")}
          </div>
          <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.6, color: "var(--ink-2)", maxWidth: 720 }}>
            {t("Buying a placement never buys coverage, and it never influences what we report. Paid content is clearly labelled and produced by DTW Studio, not our reporters. The rules are public.",
          ...
          <div style={{ marginTop: 12 }}>
            <a onClick={() => navigate("/trust/sponsored")} className="linkish" style={{ fontSize: 13, fontWeight: 600, color: "var(--accent)", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>
              {t("Read the Sponsored & Affiliate policy", "Đọc Chính sách Tài trợ & Affiliate", "Baca Kebijakan Sponsor & Afiliasi")}
      ...
      <div style={{
        ...
        padding: "34px 40px", background: "var(--banner)", color: "#E8EDF7",
        borderRadius: 12, position: "relative", overflow: "hidden"
      }}>
        <GridBackdrop color="rgba(232, 237, 247, .05)" size={32} fadeRadius="80%" />
        <div style={{ position: "absolute", left: -60, top: -60, width: 220, height: 220, borderRadius: "50%", background: "var(--accent)", opacity: .16, filter: "blur(70px)", pointerEvents: "none" }} />
        <div style={{ position: "relative" }}>
          <h2 className="serif" style={{ margin: "0 0 8px", fontSize: 28, fontWeight: 650, letterSpacing: "-0.02em", lineHeight: 1.12, color: "#FFFFFF", textWrap: "balance" }}>
            {t("Let's plan a campaign", "Cùng lên kế hoạch chiến dịch", "Mari rancang kampanye")}
          </h2>
          <p style={{ margin: 0, fontSize: 14, lineHeight: 1.6, color: "rgba(232, 237, 247, .78)", maxWidth: 480 }}>
            {t("Tell us who you want to reach and your budget. We'll send the media kit, the full rate card, and a recommended plan, usually within two business days.",
        ...
        <div style={{ position: "relative", display: "flex", flexDirection: "column", gap: 12, alignItems: "flex-start" }}>
          <a href={mailto} className="accent-glow" style={{
            ...
            justifyContent: "center", padding: "14px 22px", background: "var(--accent)",
            color: "#fff", borderRadius: 8, fontSize: 15, fontWeight: 600, textDecoration: "none"
          }}>
            <Icon name="mail" size={16} color="#fff" /> {EMAIL}
          </a>
          <div style={{ fontSize: 12, lineHeight: 1.5, color: "rgba(232, 237, 247, .62)" }}>
            {t("Media kit and rate card sent on request. Asia Press Centre Group · Singapore.",
...
Object.assign(window, { AdvertisePage });
